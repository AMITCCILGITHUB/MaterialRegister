...
public class HeatChartSheetsId implements Serializable,
		Comparable<HeatChartSheetsId> {
	...
	public HeatChartSheetsId(String chartNumber, int sheetNumber,
			String ctNumber) {
	...
	public String toString() {
		...
		return "[1]" + chartNumber.get() + "[2]" + sheetNumber.get() + "[3]"
				+ ctNumber.get();
	}
	...
	public int compareTo(HeatChartSheetsId o) {
		...
		if (o instanceof HeatChartSheetsId && o != null) {
			return this.sheetNumber.getValue().compareTo(
					o.sheetNumberProperty().getValue());
		} else {
