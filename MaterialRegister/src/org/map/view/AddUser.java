...
public class AddUser {
    ...
    public Node createView() {
        try {
            ...
            final TextBox userNameTextBox = new TextBox("", "User Name", newUser.userNameProperty(), true);
            final HBox passwordHBox = new HBox(H_SPACE);
            Label passwordLabel = new Label("Password");
            passwordLabel.setPrefWidth(LABEL_WIDTH);
            final PasswordBox passwordBox = new PasswordBox("", "Password", newUser.passwordProperty(), true);
            final HBox confirmPasswordHBox = new HBox(H_SPACE);
            Label confirmPasswordLabel = new Label("Confirm Password");
            confirmPasswordLabel.setPrefWidth(LABEL_WIDTH);
            final PasswordBox confirmPasswordBox = new PasswordBox("", "Confirm Password");
            final HBox roleHBox = new HBox(H_SPACE);
            Label roleLabel = new Label("Role");
            roleLabel.setPrefWidth(LABEL_WIDTH);
            final CustomComboBox roleChoiceBox = new CustomComboBox("", "Role", "Role", newUser.roleProperty(), true);
            userNameHBox.getChildren().addAll(userNameLabel, userNameTextBox);
            passwordHBox.getChildren().addAll(passwordLabel, passwordBox);
            confirmPasswordHBox.getChildren().addAll(confirmPasswordLabel, confirmPasswordBox);
            roleHBox.getChildren().addAll(roleLabel, roleChoiceBox);
            userDetailsVBox.getChildren().addAll(userNameHBox, passwordHBox, confirmPasswordHBox, roleHBox);
                        ...
                        if (passwordBox.getText().trim().length() > 0 && confirmPasswordBox.getText().trim().length() > 0) {
                            if (passwordBox.getText().equalsIgnoreCase(confirmPasswordBox.getText())) {
                                if (roleChoiceBox.getText().trim().length() > 0) {
                                    ...
                                    Alert.showAlert(MaterialRegister.getMaterialRegister().getPrimaryStage(), "Error", "Error", "Please select a role for user.");
                                }
                            } else {
                                Alert.showAlert(MaterialRegister.getMaterialRegister().getPrimaryStage(), "Error", "Error", "Password and confirm password does not match.");
                            }
                        } else {
                            Alert.showAlert(MaterialRegister.getMaterialRegister().getPrimaryStage(), "Error", "Error", "Either password or confirm password is empty.");
                        }
                    } else {
                        Alert.showAlert(MaterialRegister.getMaterialRegister().getPrimaryStage(), "Error", "Error", "User name is empty.");
                    }
                }
            });
