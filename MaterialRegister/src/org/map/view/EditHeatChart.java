	...
	public Node createView() {
		try {
			...
			EventHandler printEventHandler = new EventHandler<ActionEvent>() {
				...
				@Override
				public void handle(ActionEvent e) {
					heatChart.setHeatchartsheets(new HashSet<>(table.getItems()
							.subList(0, table.getItems().size())));
					try {
			...
			EventHandler undoEventHandler = new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent actionEvent) {
					final HeatChartMaster delHeatChart = tableMailbox
							.getSelectionModel().getSelectedItem();
					EventHandler<ActionEvent> delUserEvent = new EventHandler<ActionEvent>() {
						...
						public void handle(ActionEvent arg0) {
							...
							for (HeatChartSheets delHSheets : delHeatChart
									.getHeatchartsheets()) {
					...
					Confirm.showConfirm(MaterialRegister.getMaterialRegister()
							...
							"Restore " + delHeatChart.getChartNumber()
			...
			EventHandler deleteEventHandler = new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent actionEvent) {
					...
					Confirm.showConfirm(MaterialRegister.getMaterialRegister()
							.getPrimaryStage(), "Confirm", "Confirm",
							"Delete " + delHeatChart.getChartNumber()
									+ ". Are you sure?", delUserEvent);
				}
			};
			tableMailbox.setContextMenu(new TableContextMenu(undoEventHandler,
					deleteEventHandler, printEventHandler));
			...
			return scrollPane;
		} catch (HibernateException | URISyntaxException
