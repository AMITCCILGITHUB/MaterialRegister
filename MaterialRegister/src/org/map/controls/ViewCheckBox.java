...
public class ViewCheckBox extends Region {
    ...
    public ViewCheckBox(String labelText, int defValue) {
        ...
        checkBox.setText((defValue == 0) ? indeterminateTextValue : (defValue == 1) ? selectedTextValue : deselectedTextValue);
        if (defValue == 0) {
    ...
    public ViewCheckBox(String labelText, int defValue, final SimpleIntegerProperty value, boolean bidirectionnal) {
        initComponent(labelText);
        ...
        if (bidirectionnal) {
            checkBox.selectedProperty().addListener(new ChangeListener< Boolean>() {
                ...
                public void changed(ObservableValue ov, Boolean old_val,
                        Boolean new_val) {
                    value.set(checkBox.isSelected() ? 1
                            : (checkBox.isIndeterminate() ? 0 : -1));
                }
            });
        }
    ...
    private void initComponent(final String labelText) {
        ...
        setMinSize(Layout.getLabelWidth() + Layout.getCheckBoxWidth(), Layout.getRegionHeight());
        setPrefSize(Layout.getLabelWidth() + Layout.getCheckBoxWidth(), Layout.getRegionHeight());
        setMaxSize(Layout.getLabelWidth() + Layout.getCheckBoxWidth(), Layout.getRegionHeight());

        label = new Label(labelText);
        label.setPrefWidth(Layout.getLabelWidth());

        checkBox = CheckBoxBuilder.create().allowIndeterminate(true).disable(true).prefHeight(Layout.getCheckBoxHeight()).prefWidth(Layout.getCheckBoxWidth()).build();
    ...
    protected void layoutChildren() {
        ...
        checkBox.resizeRelocate(label.getPrefWidth() + 12, 0, checkBox.getPrefWidth(), getHeight());
    }
}
