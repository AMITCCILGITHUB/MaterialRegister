...
public class ETFCellFactory
	...
	public static class TextFieldCell extends
			TableCell<HeatChartSheets, String> {
		...
		protected void updateItem(String item, boolean empty) {
			...
			if (!empty) {
				...
				if (type.equalsIgnoreCase("CTNumberAdd") && item != null
						&& item.length() > 5) {
					AddHeatChart.getViewHeatChart().updateTable(
							MaterialData.searchMaterialDetailsByCtNumber(item),
							getIndex());
				}
				if (type.equalsIgnoreCase("CTNumberEdit") && item != null
						&& item.length() > 5) {
					EditHeatChart.getViewHeatChart().updateTable(
							MaterialData.searchMaterialDetailsByCtNumber(item),
							getIndex());
				}
			} else {
