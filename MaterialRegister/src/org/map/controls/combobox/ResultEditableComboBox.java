...
public class ResultEditableComboBox extends Region {
	...
	public ResultEditableComboBox() {

		resultProperty = new ResultProperty();
		initComponent();
	}

	public ResultEditableComboBox(String promptText) {
		...
		textBox.textProperty().bindBidirectional(
				resultProperty.get().resultNameProperty());
	}

	public ResultEditableComboBox(String promptText, ResultProperty result) {

		resultProperty = new ResultProperty();
		initComponent();

		textBox.setPromptText(promptText);
		resultProperty.bindBidirectional(result);
			...
			public void changed(ObservableValue<? extends String> observable,
				...
				if (!textBox.isDisabled() && textBox.isFocused() == true) {
					if (textBox.getText().length() == 0) {
						...
						List<ResultMaster> resultList = ValidationData
								.getResultList(textBox.getText().trim());

						if (resultList.size() == 1
								&& resultList.get(0).getResultCode() == 0) {

							resultProperty.set(resultList.get(0));
	...
	private void populateMenu(List<ResultMaster> resultList) {

		resultContextMenu.getItems().clear();
		Iterator<ResultMaster> results = resultList.iterator();

		while (results.hasNext()) {
			final ResultMaster result = (ResultMaster) results.next();
			final HBox hBox = new HBox();
			hBox.setFillHeight(true);
			Label itemLabel = new Label(result.getResultName());
			itemLabel.getStyleClass().add("item-label");
			hBox.getChildren().addAll(itemLabel);
