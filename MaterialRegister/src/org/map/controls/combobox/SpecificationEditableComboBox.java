...
public class SpecificationEditableComboBox extends Region {

	private SpecificationProperty specificationProperty;
	private TextField textBox;
	private Button errorButton;
	private Button editButton;
	private ContextMenu resultContextMenu = new ContextMenu();

	public SpecificationEditableComboBox() {

		specificationProperty = new SpecificationProperty();
		initComponent();
	}

	public SpecificationEditableComboBox(String promptText) {
		...
		textBox.textProperty().bindBidirectional(
				specificationProperty.get().specificationNameProperty());
	}

	public SpecificationEditableComboBox(String promptText, SpecificationProperty specification) {

		specificationProperty = new SpecificationProperty();
		initComponent();

		textBox.setPromptText(promptText);
		specificationProperty.bindBidirectional(specification);
				...
				if (!textBox.isDisabled() && textBox.isFocused() == true) {
					...
					if (textBox.getText().length() == 0) {
						...
						if (resultList.size() == 1
								&& resultList.get(0).getSpecificationCode() == 0) {

							specificationProperty.set(resultList.get(0));
	...
	private void populateMenu(List<SpecificationMaster> resultList) {

		resultContextMenu.getItems().clear();
		Iterator<SpecificationMaster> results = resultList.iterator();

		while (results.hasNext()) {
			final SpecificationMaster result = (SpecificationMaster) results.next();
			final HBox hBox = new HBox();
			hBox.setFillHeight(true);
			Label itemLabel = new Label(result.getSpecificationName());
			itemLabel.getStyleClass().add("item-label");
			hBox.getChildren().addAll(itemLabel);
			...
			menuItem.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent actionEvent) {
					...
					specificationProperty.set(result);
				}
			});
		}
	}
